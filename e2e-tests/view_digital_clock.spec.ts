...
test.only('should able to impersonate customer and add digital clock views on playlist', async ({ page }) => {
  ...
  //fill infomation
  await page.locator('div').filter({ hasText: /^Digital Clock$/ }).locator('svg').click();
  await page.locator('div').filter({ hasText: /^Select\.\.\.$/ }).nth(2).click();
  await page.getByText('Alaska Time').click();
  await page.getByPlaceholder('Enter Name').click();
  await page.getByPlaceholder('Enter Name').fill('test 3 fix');
