...
test.only('should able to impersonate customer and add digital word clock views on playlist', async ({ page }) => {
  ...
  //fill infomation
await page.getByText('Digital World Clock').click();
  await page.locator('div').filter({ hasText: /^Select\.\.\.$/ }).nth(2).click();
  await page.locator('li').filter({ hasText: 'Alaska Time' }).click();
  await page.getByPlaceholder('Enter Name').click();
  await page.getByPlaceholder('Enter Name').fill('automated test 4');
