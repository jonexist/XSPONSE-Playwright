...
test.describe('impersonate Customer and search views by name on playlist', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/account-management/customer');
  });

test('should able to impersonate customer and search views by name on playlist', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search' }).fill('test del1');

// Get all matching elements
const allTest2Elements = await page.getByText('test del1').all();
