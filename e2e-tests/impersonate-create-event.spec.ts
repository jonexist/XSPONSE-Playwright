...
test.describe('log in as admin and impersonate customer, Event creation – basic flow', () => {
...
test.only('log in as admin and impersonate customer, Event creation – basic flow', async ({ page }) => {
  ...
  await page.getByTestId('customer-search-input').fill('Lee');
  await page.getByTestId('customer-search-input').press('Enter');
  await page.locator('#impersonate-user-5').click();
