...
test('should able to impersonate customer and add views on playlist', async ({ page }) => {
  ...
  //fill infomation
  await page.locator('.grid > div > svg').first().click();
  await page.locator('div').filter({ hasText: /^Select\.\.\.$/ }).nth(2).click();
  await page.getByPlaceholder('Search...').fill('Philippine Time');
  await page.getByText('Philippine Time').click();
  await page.getByPlaceholder('Name').click();
  await page.getByPlaceholder('Name').fill('sample playlist test 1');

  await page.getByPlaceholder('Custom Label').click();
  await page.getByPlaceholder('Custom Label').fill('test1');
